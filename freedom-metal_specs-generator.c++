...
memory::memory(std::string mtype, std::string alias, std::string name, int base,
...
static void dts_memory(void) {
  ...
  for (offset = fdt_next_node(dts_blob, offset, &depth);
    ...
    regnamep =
        (const char *)fdt_getprop(dts_blob, offset, "reg-names", &regnamelen);
    if (regnamelen > 0) {
      const char *regp;
      int reglen, regidx;
      s = regnamep;
      ...
      do {
        ...
        if (reglen > 0) {
          ...
          dts_memory_list.push_back(memory(s, splices[0], splices[0], base,
...
static void write_specs_file(fstream &os, std::string machine,
                             std::string prefix, std::string tuple,
  ...
  os << "*cpp:\n";
  os << "-D__METAL_MACHINE_HEADER=\"metal/machine/" << machine
     << ".h\" %(metal_machine__cpp)\n";
  os << "\n";

  os << "%rename cc1  metal_machine__cc1\n";
  os << "*cc1:\n";
  os << "-ffunction-sections -fdata-sections -march=" << isa << " -mabi=" << abi
     << " %(metal_machine__cc1)\n";
  os << "\n";
  ...
  os << "*cc1plus:\n";
  os << "-ffunction-sections -fdata-sections -march=" << isa << " -mabi=" << abi
     << " %(metal_machine__cc1plus)\n";
  os << "\n";

  os << "*startfile:\n";
  os << "%{!nostartfiles:" << prefix << "/" << tuple << "/lib/" << isa << "/"
     << abi << "/crt0__metal.o"
     << " " << prefix << "/lib/gcc/" << tuple << "/" << gcc_version << "/"
     << isa << "/" << abi << "/crtbegin.o}\n";
  os << "\n";

  os << "*endfile:\n";
  os << "%{!nostartfiles:" << prefix << "/lib/gcc/" << tuple << "/"
     << gcc_version << "/" << isa << "/" << abi << "/crtend.o}\n";
  os << "\n";
  ...
  os << "%{!T*:-Triscv__mmachine__" << machine << ".lds}"
     << " --gc-sections"
     << " -march=" << isa << " -mabi=" << abi << " -m" << emul << " -L"
     << prefix << "/" << tuple << "/lib/" << isa << "/" << abi << "/"
     << " -L" << prefix << "/lib/gcc/" << tuple << "/" << gcc_version << "/"
     << isa << "/" << abi << "/"
     << " %{!nostdlib:-lgcc}"
     << " %(metal_machine__link)\n";
  os << "\n";
