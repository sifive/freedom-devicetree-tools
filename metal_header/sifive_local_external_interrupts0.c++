...
sifive_local_external_interrupts0::sifive_local_external_interrupts0(
  ...
  dtb.match(std::regex(compat_string),
            [&](node n) { num_local_ext_ints += 1; });
}

void sifive_local_external_interrupts0::create_defines() {
...
void sifive_local_external_interrupts0::include_headers() {
  dtb.match(std::regex(compat_string),
            [&](node n) { emit_include(compat_string); });
}

void sifive_local_external_interrupts0::declare_inlines() {
  Inline *func;
...
void sifive_local_external_interrupts0::define_inlines() {
  Inline *func;
  std::list<Inline *> extern_inlines;
    ...
    n.maybe_tuple_index(
        ...
        [&](int i, uint32_t irline) {
          if ((count == 0) && (i == 0)) {
            ...
            extern_inlines.push_back(func);
          } else if (((count + 1) == num_local_ext_ints) &&
                     ((i + 1) == max_interrupts)) {
...
void sifive_local_external_interrupts0::declare_structs() {
  dtb.match(std::regex(compat_string), [&](node n) {
    emit_struct_decl("sifive_local_external_interrupts0", n);
  });
}

void sifive_local_external_interrupts0::define_structs() {
  dtb.match(std::regex(compat_string), [&](node n) {
    emit_struct_begin("sifive_local_external_interrupts0", n);

    emit_struct_field("irc.vtable", "&__metal_driver_vtable_sifive_local_"
                                    "external_interrupts0.local0_vtable");
