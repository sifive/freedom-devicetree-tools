...
sifive_rtc1::sifive_rtc1(std::ostream &os, const fdt &dtb)
    : Device(os, dtb, "sifive,(rtc1|aon0)") {
  num_rtcs = 0;
  dtb.match(std::regex(compat_string), [&](node n) { num_rtcs += 1; });
}

void sifive_rtc1::include_headers() {
  dtb.match(std::regex(compat_string),
            [&](node n) { emit_include("sifive,rtc1"); });
}

void sifive_rtc1::declare_inlines() {
  Inline *func;
  ...
  while (!extern_inlines.empty()) {
    ...
    emit_inline_dec(func, "sifive_rtc1");
    delete func;
  }
  os << "\n";
}

void sifive_rtc1::define_inlines() {
  Inline *base_func;
  Inline *size_func;
  ...
  dtb.match(std::regex(compat_string), [&](node n) {
    ...
    if (n.field_exists("interrupts")) {
      /* If the compatible string is "sifive,rtc1", then there should only be
  ...
  if (num_rtcs != 0) {
    ...
    emit_inline_def(base_func, "sifive_rtc1");
    delete base_func;
    emit_inline_def(size_func, "sifive_rtc1");
    delete size_func;
    emit_inline_def(int_parent_func, "sifive_rtc1");
    delete int_parent_func;
    emit_inline_def(int_line_func, "sifive_rtc1");
    delete int_line_func;
    emit_inline_def(clock_func, "sifive_rtc1");
    delete clock_func;
  }
  os << "\n";
}

void sifive_rtc1::declare_structs() {
  dtb.match(std::regex(compat_string), [&](node n) {
    emit_comment(n);
    os << "extern struct __metal_driver_sifive_rtc1 __metal_dt_rtc_"
       << n.instance() << ";\n\n";
  });
}

void sifive_rtc1::define_structs() {
  dtb.match(std::regex(compat_string), [&](node n) {
    emit_comment(n);
    os << "struct __metal_driver_sifive_rtc1 __metal_dt_rtc_" << n.instance()
       << " = {\n";

    emit_struct_field("rtc.vtable", "&__metal_driver_vtable_sifive_rtc1.rtc");

    emit_struct_end();
  });
}

void sifive_rtc1::create_handles() {
  emit_def("__METAL_DT_MAX_RTC1S", std::to_string(num_rtcs));

  emit_struct_pointer_begin("sifive_rtc1", "__metal_rtc1_table", "[]");
  if (num_rtcs) {
    int i = 0;
    dtb.match(std::regex(compat_string), [&](node n) {
