...
void sifive_wdog0::define_inlines()
{
  ...
  dtb.match(
	...
	/* If the compatible string is "sifive,wdog0", then there should only be one interrupt line.
	 * If the compatible string is "sifive,aon0", the first interrupt line corresponds to the
	 * watchdog interrupt. */
	int_line_value = std::to_string(n.get_fields<uint32_t>("interrupts").at(0)); 
      }
...
void sifive_wdog0::declare_structs()
{
  dtb.match(
    std::regex(compat_string),
    [&](node n) {
      emit_struct_decl("sifive_wdog0", n);
    }
  );
}
...
void sifive_wdog0::define_structs()
{
  dtb.match(
    ...
    [&](node n) {
      ...
      emit_struct_field("watchdog.vtable", "&__metal_driver_vtable_sifive_wdog0.watchdog");
