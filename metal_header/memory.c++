...
memory::memory(std::ostream &os, const fdt &dtb)
  : Device(os, dtb, "")
{
  num_memories = 0;
  dtb.match(
      std::regex("sifive,sram0"),    [&](node n) { num_memories += 1; },
...
void memory::include_headers()
{
  os << "#include <metal/memory.h>\n";
}
...
void memory::declare_structs()
{
  ...
  dtb.match(
    std::regex("sifive,sram0"), declare,
    std::regex("sifive,testram0"), declare,
    std::regex("sifive,dtim0"), declare,
    std::regex("sifive,itim0"), declare,
    std::regex("memory"), declare,
...
void memory::define_structs()
{
  ...
  dtb.match(
      std::regex("sifive,sram0"), define,
      std::regex("sifive,testram0"), define,
      std::regex("sifive,dtim0"), define,
      std::regex("sifive,itim0"), define,
      std::regex("memory"), define,
...
void memory::create_handles()
{
  ...
  if (num_memories) {
    dtb.match(
      std::regex("sifive,sram0"), emit,
      std::regex("sifive,testram0"), emit,
      std::regex("sifive,dtim0"), emit,
      std::regex("sifive,itim0"), emit,
      std::regex("memory"), emit,
      std::regex("sifive,spi0"),
	[&](node n) {
