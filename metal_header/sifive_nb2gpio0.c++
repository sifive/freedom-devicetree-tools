...
sifive_nb2gpio0::sifive_nb2gpio0(std::ostream &os, const fdt &dtb)
...
void sifive_nb2gpio0::create_defines()
{
  dtb.match(
    std::regex(compat_string),
    [&](node n) {
...
void sifive_nb2gpio0::include_headers()
{
  dtb.match(
    std::regex(compat_string),
    [&](node n) {
...
void sifive_nb2gpio0::declare_inlines()
{
  Inline* func;
  ...
  while (!extern_inlines.empty()) {
    ...
    emit_inline_dec(func, "sifive_nb2gpio0");
    delete func;
  }
  os << "\n";
}

void sifive_nb2gpio0::define_inlines()
{
  Inline* func;
  ...
  while (!extern_inlines.empty()) {
    func = extern_inlines.front();
    extern_inlines.pop_front();
    emit_inline_def(func, "sifive_nb2gpio0");
    delete func;
  }
  os << "\n";
}

void sifive_nb2gpio0::declare_structs()
{
  dtb.match(
    std::regex(compat_string),
    [&](node n) {
      emit_struct_decl("sifive_nb2gpio0", n);
    }
  );
}

void sifive_nb2gpio0::define_structs()
{
  dtb.match(
    std::regex(compat_string),
    [&](node n) {
      emit_struct_begin("sifive_nb2gpio0", n);

      emit_struct_field("gpio.vtable", "&__metal_driver_vtable_sifive_nb2gpio0.gpio");

      emit_struct_end();
    });
}

void sifive_nb2gpio0::create_handles()
{
  emit_def("__METAL_DT_MAX_GPIOS", std::to_string(num_gpios));

  emit_struct_pointer_begin("sifive_nb2gpio0", "__metal_gpio_table", "[]");
  if (num_gpios) {
    int i = 0;
    dtb.match(
      std::regex(compat_string),
