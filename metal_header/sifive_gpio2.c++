...
sifive_gpio2::sifive_gpio2(std::ostream &os, const fdt &dtb)
...
void sifive_gpio2::create_defines() {
  ...
  emit_def("METAL_MAX_GPIO2_INTERRUPTS", std::to_string(max_interrupts));
}

void sifive_gpio2::include_headers() {
  dtb.match(std::regex(compat_string),
            [&](node n) { emit_include(compat_string); });
}

void sifive_gpio2::declare_inlines() {
  Inline *func;
  ...
  while (!extern_inlines.empty()) {
    ...
    emit_inline_dec(func, "sifive_gpio2");
    delete func;
  }
  os << "\n";
}

void sifive_gpio2::define_inlines() {
  Inline *func;
    ...
    if (num_gpios == 0) {
      ...
      if (count == 0) {
        ...
        func3 = create_inline_def(
            "num_interrupts", "int",
            "(uintptr_t)gpio == (uintptr_t)&__metal_dt_" + n.handle(),
            "METAL_MAX_GPIO2_INTERRUPTS", "struct metal_gpio *gpio");
  ...
  while (!extern_inlines.empty()) {
    func = extern_inlines.front();
    extern_inlines.pop_front();
    emit_inline_def(func, "sifive_gpio2");
    delete func;
  }
  os << "\n";
}

void sifive_gpio2::declare_structs() {
  dtb.match(std::regex(compat_string),
            [&](node n) { emit_struct_decl("sifive_gpio2", n); });
}

void sifive_gpio2::define_structs() {
  dtb.match(std::regex(compat_string), [&](node n) {
    emit_struct_begin("sifive_gpio2", n);

    emit_struct_field("gpio.vtable",
                      "&__metal_driver_vtable_sifive_gpio2.gpio");

    emit_struct_end();
  });
}

void sifive_gpio2::create_handles() {
  emit_def("__MEE_DT_MAX_GPIOS2", std::to_string(num_gpios));

  emit_struct_pointer_begin("sifive_gpio2", "__metal_gpio2_table", "[]");
  if (num_gpios) {
    int i = 0;
    dtb.match(std::regex(compat_string), [&](node n) {
