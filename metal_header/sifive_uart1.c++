...
#include <sifive_uart1.h>

sifive_uart1::sifive_uart1(std::ostream &os, const fdt &dtb)
    : Device(os, dtb, "sifive,uart1") {}

void sifive_uart1::create_defines() {
...
void sifive_uart1::declare_inlines() {
  Inline *func;
  ...
  while (!extern_inlines.empty()) {
    ...
    emit_inline_dec(func, "sifive_uart1");
    delete func;
  }
  os << "\n";
}

void sifive_uart1::define_inlines() {
  Inline *base_func;
  Inline *size_func;
  Inline *num_int_func;
  ...
  if (num_uarts != 0) {
    ...
    emit_inline_def(base_func, "sifive_uart1");
    delete base_func;
    emit_inline_def(size_func, "sifive_uart1");
    delete size_func;
    emit_inline_def(num_int_func, "sifive_uart1");
    delete num_int_func;
    emit_inline_def(int_parent_func, "sifive_uart1");
    delete int_parent_func;
    emit_inline_def(int_line_func, "sifive_uart1");
    delete int_line_func;
    emit_inline_def(clock_func, "sifive_uart1");
    delete clock_func;
    emit_inline_def(pinmux_func, "sifive_uart1");
    delete pinmux_func;
    emit_inline_def(pinmux_out_func, "sifive_uart1");
    delete pinmux_out_func;
    emit_inline_def(pinmux_source_func, "sifive_uart1");
    delete pinmux_source_func;
  }
  os << "\n";
}

void sifive_uart1::declare_structs() {
  dtb.match(std::regex(compat_string),
            [&](node n) { emit_struct_decl("sifive_uart1", n); });
}

void sifive_uart1::define_structs() {
  dtb.match(std::regex(compat_string), [&](node n) {
    emit_struct_begin("sifive_uart1", n);

    emit_struct_field("uart.vtable",
                      "&__metal_driver_vtable_sifive_uart1.uart");
