...
sifive_fe310_g000_lfrosc::sifive_fe310_g000_lfrosc(std::ostream &os, const fdt &dtb)
...
void sifive_fe310_g000_lfrosc::declare_inlines()
{
  ...
  dtb.match(
    ...
    [&](node n) {
      if (count == 0) {
	func = create_inline_dec("lfrosc",
				 "struct metal_clock *",
				 "const struct metal_clock *clock");
	extern_inlines.push_back(func);

	func = create_inline_dec("psdlfaltclk",
				 "struct metal_clock *",
				 "const struct metal_clock *clock");
	extern_inlines.push_back(func);

	func = create_inline_dec("config_reg",
				 "unsigned long int",
				 "const struct metal_clock *clock");
	extern_inlines.push_back(func);

	func = create_inline_dec("mux_reg",
				 "unsigned long int",
				 "const struct metal_clock *clock");
	extern_inlines.push_back(func);
      }
...
void sifive_fe310_g000_lfrosc::define_inlines()
{
  ...
  dtb.match(
    ...
    [&](node n) {
      ...
      n.named_tuples(
	"clock-names", "clocks",
	"lfrosc", tuple_t<node>(), [&](node m) {
	  lfrosc_node = "(struct metal_clock *)&__metal_dt_"
		       + m.handle() + ".clock";
        },
	"psdlfaltclk", tuple_t<node>(), [&](node m) {
	...
	[&](node m, target_size offset) {
	  ...
	  m.named_tuples(
	    "reg-names", "reg",
	    "control", tuple_t<target_addr, target_size>(), [&](target_addr b, target_size size) {
		base = b;
	    });
...
void sifive_fe310_g000_lfrosc::declare_structs()
{
  dtb.match(
    std::regex(compat_string),
    [&](node n) {
      emit_struct_decl("sifive_fe310_g000_lfrosc", n);
    }
  );
}
...
void sifive_fe310_g000_lfrosc::define_structs()
{
  dtb.match(
    ...
    [&](node n) {
      ...
      emit_struct_field("clock.vtable", "&__metal_driver_vtable_sifive_fe310_g000_lfrosc.clock");
