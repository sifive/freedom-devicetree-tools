	...
	[&](node n) {
	  ...
	  n.named_tuples(
	    ...
	    "config", tuple_t<node, target_size>(), [&](node base, target_size off) {
	      ...
	      emit_struct_field_platform_define_offset("sifive,fe310-g000,prci", "config_offset", base, METAL_PLLCFG_LABEL);
	    },
	    "divider", tuple_t<node, target_size>(), [&](node base, target_size off) {
	      emit_struct_field_node("divider_base", base, "");
	      emit_struct_field_platform_define_offset("sifive,fe310-g000,prci", "divider_offset", base, METAL_PLLOUTDIV_LABEL);
	    });
