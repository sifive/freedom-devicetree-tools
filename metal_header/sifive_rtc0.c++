...
void sifive_rtc0::define_inlines()
{
  ...
  dtb.match(
	...
	/* If the compatible string is "sifive,rtc0", then there should only be one interrupt line.
	 * If the compatible string is "sifive,aon0", the second interrupt line corresponds to the
	 * rtc interrupt. */
	int_line_value = std::to_string(n.get_fields<uint32_t>("interrupts").back()); 
      }
