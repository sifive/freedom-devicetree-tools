...
class sifive_local_external_interrupts0 : public Device {
  public:
	...
	std::regex(compat_string),
	[&](node n) {
	  emit_struct_decl("sifive_local_external_interrupts0_data", n, "_data");
	  emit_const_struct_decl("sifive_local_external_interrupts0", n);
	}
      );
    }
	...
	std::regex(compat_string),
	[&](node n) {
	  emit_struct_begin("sifive_local_external_interrupts0_data", n, "_data");
	  emit_struct_field("init_done", "0");
	  emit_struct_end();

	  emit_const_struct_begin("sifive_local_external_interrupts0", n);
