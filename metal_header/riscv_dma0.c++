...
void riscv_dma0::declare_inlines() {
  ...
  dtb.match(std::regex(compat_string), [&](node n) {
    if (count == 0) {
      ...
      func =
          create_inline_dec("num_interrupts", "int", "struct metal_pdma *dma");
      extern_inlines.push_back(func);
      ...
      func = create_inline_dec("interrupt_lines", "int",
                               "struct metal_pdma *dma", "int idx");
      extern_inlines.push_back(func);

      func =
          create_inline_dec("interrupt_line", "int", "struct metal_pdma *dma");
      extern_inlines.push_back(func);
          ...
          if ((count == 0) && (i == 0)) {
            interrupt_line_func = create_inline_def(
                ...
                "((uintptr_t)dma == (uintptr_t)&__metal_dt_" + n.handle() +
                    ") && (" + "idx == " + std::to_string(i) + ")",
                std::to_string(irline), "struct metal_pdma *dma", "int idx");
...
void riscv_dma0::define_structs() {
  dtb.match(std::regex(compat_string), [&](node n) {
    ...
    emit_struct_field("dma.vtable", "&__metal_driver_vtable_riscv_dma0.dma");
...
void riscv_dma0::create_handles() {
  ...
  emit_struct_pointer_begin("riscv_dma0", "__metal_pdma_table", "[]");
  if (num_dmas) {
    int i = 0;
    dtb.match(std::regex(compat_string), [&](node n) {
