...
sifive_simuart0::sifive_simuart0(std::ostream &os, const fdt &dtb)
  : Device(os, dtb, "sifive,simuart0")
{}

void sifive_simuart0::create_defines()
{
  dtb.match(
  ...
  emit_def("METAL_MAX_SIMUART_INTERRUPTS", std::to_string(max_interrupts));
}

void sifive_simuart0::include_headers()
{
  dtb.match(
    std::regex(compat_string),
    [&](node n) {
...
void sifive_simuart0::declare_inlines()
{
  Inline* func;
  ...
  while (!extern_inlines.empty()) {
    ...
    emit_inline_dec(func, "sifive_simuart0");
    delete func;
  }
  os << "\n";
}

void sifive_simuart0::define_inlines()
{
  Inline* base_func;
  Inline* size_func;
  Inline* num_int_func;
  ...
  dtb.match(
    ...
    [&](node n) {
      ...
      n.maybe_tuple(
        ...
        [&](){},
        [&](uint32_t irline){
          int_line_value = std::to_string(irline);
        });

      /* Clock driving the SIMUART peripheral */
      ...
      n.maybe_tuple(
        ...
        [&](node m, uint32_t dest, uint32_t source) {
          pinmux_value = "(struct __metal_driver_sifive_gpio0 *)&__metal_dt_" + m.handle();
          pinmux_dest = dest;
      ...
      if (count == 0) {
        ...
        num_int_func = create_inline_def("num_interrupts",
                                         "int",
                                         "(uintptr_t)uart == (uintptr_t)&__metal_dt_" + n.handle(),
                                         "METAL_MAX_SIMUART_INTERRUPTS",
                                         "struct metal_uart *uart");

        int_parent_func = create_inline_def("interrupt_parent",
                                            "struct metal_interrupt *",
                                            "(uintptr_t)uart == (uintptr_t)&__metal_dt_" + n.handle(),
                                            int_parent_value,
                                            "struct metal_uart *uart");

        int_line_func = create_inline_def("interrupt_line",
                                          "int",
                                          "(uintptr_t)uart == (uintptr_t)&__metal_dt_" + n.handle(),
                                          int_line_value,
                                          "struct metal_uart *uart");
        ...
        pinmux_out_func = create_inline_def("pinmux_output_selector",
                                 "unsigned long",
                                 "(uintptr_t)uart == (uintptr_t)&__metal_dt_" + n.handle(),
                                 std::to_string(pinmux_dest),
                                 "struct metal_uart *uart");

        pinmux_source_func = create_inline_def("pinmux_source_selector",
                                 "unsigned long",
                                 "(uintptr_t)uart == (uintptr_t)&__metal_dt_" + n.handle(),
  ...
  if (num_uarts != 0) {
    ...
    emit_inline_def(base_func, "sifive_simuart0");
    delete base_func;
    emit_inline_def(size_func, "sifive_simuart0");
    delete size_func;
    emit_inline_def(num_int_func, "sifive_simuart0");
    delete num_int_func;
    emit_inline_def(int_parent_func, "sifive_simuart0");
    delete int_parent_func;
    emit_inline_def(int_line_func, "sifive_simuart0");
    delete int_line_func;
    emit_inline_def(clock_func, "sifive_simuart0");
    delete clock_func;
    emit_inline_def(pinmux_func, "sifive_simuart0");
    delete pinmux_func;
    emit_inline_def(pinmux_out_func, "sifive_simuart0");
    delete pinmux_out_func;
    emit_inline_def(pinmux_source_func, "sifive_simuart0");
    delete pinmux_source_func;
  }
  os << "\n";
}

void sifive_simuart0::declare_structs()
{
  dtb.match(
    std::regex(compat_string),
    [&](node n) {
      emit_struct_decl("sifive_simuart0", n);
    }
  );
}
...
void sifive_simuart0::define_structs()
{
  dtb.match(
    ...
    [&](node n) {
      ...
      emit_struct_field("uart.vtable", "&__metal_driver_vtable_sifive_simuart0.uart");
