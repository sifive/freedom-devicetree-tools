...
ScratchpadLayout::ScratchpadLayout(const fdt &dtb, Memory rom_memory,
  ...
  section_groups.push_back(
      InitTextGroup(data_memory, ram_phdr, data_memory, ram_phdr));

  section_groups.push_back(
      TextGroup(data_memory, ram_phdr, data_memory, ram_phdr));

  section_groups.push_back(
      RodataGroup(data_memory, ram_phdr, data_memory, ram_phdr));

  section_groups.push_back(
      CtorsGroup(data_memory, ram_phdr, data_memory, ram_phdr));

  section_groups.push_back(ItimGroup(
      data_memory,
      ram_phdr, /* ITIM code exists in the ROM, located in the data memory */
      itim_memory,
      itim_init_phdr)); /* And is copied into the ITIM at program load */

  section_groups.push_back(
      DataGroup(data_memory, ram_phdr, data_memory, ram_init_phdr, tls_phdr));

  section_groups.push_back(
      UninitGroup(dtb, data_memory, ram_phdr, data_memory, ram_phdr, tls_phdr));
}
